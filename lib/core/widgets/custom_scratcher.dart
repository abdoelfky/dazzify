...
/// Custom Scratcher widget which covers given child with scratchable overlay.
/// This version fixes horizontal/vertical gesture detection issues.
class CustomScratcher extends StatefulWidget {
  ...
  @override
  CustomScratcherState createState() => CustomScratcherState();
}

class CustomScratcherState extends State<CustomScratcher> {
    ...
    return FutureBuilder<ui.Image?>(
      ...
      builder: (BuildContext context, AsyncSnapshot<ui.Image?> snapshot) {
        if (snapshot.connectionState != ConnectionState.waiting) {
          // Use RawGestureDetector with custom PanGestureRecognizer
          // that accepts all directions immediately without requiring movement threshold
...
/// Custom PanGestureRecognizer that accepts all directions immediately
/// without requiring a movement threshold. This fixes the issue where
/// horizontal scratching doesn't work properly.
class _ImmediatePanGestureRecognizer extends PanGestureRecognizer {
  _ImmediatePanGestureRecognizer() : super() {
    // Accept gestures in all directions without threshold
    // This ensures both horizontal and vertical scratches work immediately
  }

  @override
  void addAllowedPointer(PointerDownEvent event) {
    super.addAllowedPointer(event);
    // Immediately accept this gesture to win the gesture arena
    // This prevents parent scrollable widgets from intercepting the gesture
    resolve(GestureDisposition.accepted);
  }

  @override
  String get debugDescription => 'immediate_pan';
}
